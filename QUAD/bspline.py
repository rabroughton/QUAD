...
class Bspline():
    ...
    def __diff_internal(self):
        '''
        Differentiate a B-spline once, and return the resulting
        coefficients and Bspline objects. This preserves the Bspline
        object nature of the data, enabling recursive implementation
        of higher-order differentiation (see `diff`). The value of
        the first derivative of `B` at a point `x` can be obtained as
    ...
    def diff(self, order=1):
        ...
        # identically zero, but force the same output format
        ...
        if order > self.p:
            ...
            return lambda x: np.zeros((nbasis,), dtype=dummy.dtype)
        '''
        At each differentiation, each term maps into two new terms.
        The number of terms in the result will be 2**order.

        This will cause an exponential explosion in the number of terms
        for high derivative orders, but for the first few orders
        (practical usage; >3 is rarely needed) the approach works.
        '''
        terms = [(1., self)]
        for k in range(order):
            ...
            for Ci, Bi in terms:
                ...
                tmp.extend((Ci*cn, Bn) for cn, Bn in Bi.__diff_internal())
            terms = tmp
    ...
    def collmat(self, tau, deriv_order=0):
        ...
        Args:
            ...
            * **deriv_order** (:py:class:`int`):
                >=0, order of derivative for which to compute
        ...
        Returns:
            * **A** (rank-2 array):
                ...
                where :math:`D^k` is the :math:`k^{th}` derivative
                (0 for function value itself)

        .. note::
            If the coefficients of a spline function are given in
            the vector c, then
            ...
            will give a rank-1 array of function values at the sites **tau**
            that were supplied to :func:`collmat`.
