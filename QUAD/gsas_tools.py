...
class Calculator:
    ...
    '''
        ################################################## How it works #################################################
        #
        # Variables are stored in various python dictionaries (parmDict, calcControls, Phases, and Histograms)
        # for more information about dictionaries see http://www.tutorialspoint.com/python/python_dictionary.htm
        #
        ...
        # available models are: chebyschev (default), cosine, lin interpolate, inv interpolate, and log interpolate
        #
        ...
        # Change between isotropic and anisotropic Debye-Waller factors: Phases[Phases.keys()[0]]['Atoms'][X][9] = 'I' (isotropic) or 'A' (anisotropic) [X represents atom number (0 to n-1)]
        # Debye-Waller factors are controled by: Phases[Phases.keys()[0]]['Atoms'][X][10-16] (isotropic case only used element 10)
        #
        # Histograms contains information about the diffraction data file (this should not be changed)
        #
        #
        # This script calls the neccessary functions for QUAD from the GSAS-II program which are regularly updated by the GSAS-II developers.
        ##################################################################################################################
    '''
    def __init__(self,GPXfile=None):
        ...
        self._rbIds = rbIds
        try:
            self._lowerLimit = Histograms[list(Histograms.keys())[0]][u'Limits'][1][0]
            self._upperLimit = Histograms[list(Histograms.keys())[0]][u'Limits'][1][1]
            self._tth = Histograms[list(Histograms.keys())[0]]['Data'][0][0:-1]
            self._tthsample = self._tth[np.where((self._tth>self._lowerLimit) & (self._tth<self._upperLimit) == True)]
...
#print 'variable initallizing time: %.3fs...Successful'%(time.time()-time0)
    ...
    def Calculate(self):
        ...
        '''
            ...
            rbIds = self._rbIds
        '''
        #getPowderProfile(parmDict,x,varylist,Histogram,Phases,calcControls,pawleyLookup):
        yc1 = G2strMath.getPowderProfile(self._parmDict,self._tthsample,self._varyList,self._Histograms[list(self._Histograms.keys())[0]],self._Phases,self._calcControls,self._pawleyLookup)[0]

        return yc1
    
    def CalculateSXtal(self):
        G2strMath.GetFobsSq(self._Histograms,self._Phases,self._parmDict,self._calcControls)
...
#                    FoSig.append(self._Histograms[key]['Data']['RefList'][hkl][6])
    ...
    def CalculateDeriv(self):
        ...
        return np.sum(G2strMath.getPowderProfileDervMP(self._parmDict,self._tthsample,self._varyList,self._Histograms[self._Histograms.keys()[0]],self._Phases,self._rigidbodyDict,self._calcControls,self._pawleyLookup), axis=1)
    
    def CalculateHessian(self):
    
        values =  np.array(G2strMath.Dict2Values(self._parmDict, self._varyList))

        Z = G2strMath.HessRefine(values, [self._Histograms,self._Phases,self._restraintDict,self._rigidbodyDict],self._parmDict,self._varyList,self._calcControls,self._pawleyLookup,None)
    ...
    def CalculateJacob(self):
        ...
        values =  np.array(G2strMath.Dict2Values(self._parmDict, self._varyList))
        Y = G2strMath.dervRefine(values, [self._Histograms,self._Phases,self._restraintDict,self._rigidbodyDict],self._parmDict,self._varyList,self._calcControls,self._pawleyLookup,None)
...
#        return np.sum(Y, axis=1)
 ...
 # There are alternate methods to pull the covariance matrix out
    def getCovMatrix(self,ParmVarDict):
        #[self._Histograms,self._Phases,self._restraintDict,self._rigidbodyDict],self._parmDict,ParmVarDict,self._calcControls,self._pawleyLookup
        '''
        ...
        '''
        HistoPhases = self._Histograms,self._Phases,self._restraintDict,self._rigidbodyDict
        xtol=1.e-6
        
        values =  np.array(G2strMath.Dict2Values(self._parmDict, ParmVarDict))
        Chisq = G2strMath.errRefine(values,HistoPhases,self._parmDict,ParmVarDict,self._calcControls,self._pawleyLookup,None)
        Nobs = len(self.Calculate())
        Chisq = np.sum(Chisq**2)
        GOF = np.sqrt(Chisq/(Nobs-len(ParmVarDict)))
        
        Yvec,Amat = G2strMath.HessRefine(values,HistoPhases,self._parmDict,ParmVarDict,self._calcControls,self._pawleyLookup,None)
        Adiag = np.sqrt(np.diag(Amat))
        Anorm = np.outer(Adiag,Adiag)
        Amat = np.array(Amat/Anorm)        
        Bmat,Nzero = self.pinv(Amat)    #Moore-Penrose inversion (via SVD) & count of zeros
        Bmat = Bmat/Anorm
...
#            outputFile.write( str(x[i]) + '\t' + str(yc[i]) + '\t' + str(yb[i]) + '\n')
...
# Cubic, Tetragonal, Hexagonal, Rhombohedral, Orthorhombic, Monoclinic, Triclinic
        ...
        LatticeUpdate = {'0::A0':G[0,0], '0::A1':G[1,1], '0::A2':G[2,2], '0::A3':G[0,1], '0::A4':G[0,2], '0::A5':G[1,2]}
        #print G
